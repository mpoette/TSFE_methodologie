                        ...
                        for optuna_choice in optuna_choices:
                            try:
                                ...
                                if config_models.extraction_type == 'TSFEL':
                                    ...
                                    # Run correlation analysis and estimate optimal threshold.
                                    update_progress("Running correlation analysis on TSFEL features")
                                    tsfel_features_for_corr = [
                                        col for col in complete_tsfel_df.columns
                                        if col not in [extract.ID_COL, target_col]
                                    ]
                                    corr_df = complete_tsfel_df.select(
                                        tsfel_features_for_corr
                                    ).to_pandas()
                                    ...
                                    optimal_corr_threshold = corr_results["optimal_threshold"]
                                    np.save(
                                        compare_figs_dir / "optimal_corr_threshold.npy",
                                        np.array(optimal_corr_threshold),
                                    )
                                    print(
                                        f"Optimal correlation threshold saved: "
                                        f"{optimal_corr_threshold:.2f}"
                                    )
                                    ...
                                    # Load the optimal correlation threshold if available.
                                    if config_models.extraction_type == "TSFEL":
                                        compare_figs_dir = exp.get_compare_figs_path()
                                        threshold_path = compare_figs_dir / "optimal_corr_threshold.npy"
                                        if threshold_path.exists():
                                            fold_processing_config["corr_threshold"] = float(
                                                np.load(threshold_path)
                                            )
                                            print(
                                                f"Loaded correlation threshold: "
                                                f"{fold_processing_config['corr_threshold']:.2f}"
                                            )
                                        else:
                                            fold_processing_config["corr_threshold"] = 0.95
                                            print(
                                                "No saved threshold found, "
                                                "using default (0.95)."
                                            )
