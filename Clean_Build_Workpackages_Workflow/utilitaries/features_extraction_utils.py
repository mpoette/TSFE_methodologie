...
from utilitaries.feature_trace import record_removed_features
...
# ============================================================================
# VALIDATION AND LOW-LEVEL NUMERICAL HELPERS
# ============================================================================
...
def _log_memory(label: str) -> None:
    ...
    if resource is None:
        logger.debug(
            "[MEMORY] %s: pid=%d, memory information unavailable "
            "on this platform",
            label,
            os.getpid(),
        )
...
# ============================================================================
# DATAFRAME VALIDATION AND FEATURE SANITIZATION
# ============================================================================
...
def filtrage_corr_var(
    ...
    feature_trace_path: str | os.PathLike | None = None,
    ...
    Args:
        ...
        feature_trace_path:
            Optional JSON path recording features removed for zero variance or
            correlation, for use by explainability figures.
    ...
    numeric_train_dynamic = (
        train_dynamic
        .apply(pd.to_numeric, errors="coerce")
        .replace([np.inf, -np.inf], np.nan)
    )
    valid_dynamic_features = set(
        numeric_train_dynamic.columns[
            numeric_train_dynamic.notna().any(axis=0)
        ]
    )
    ...
    zero_variance_features = sorted(
        valid_dynamic_features.difference(var_features)
    )
    ...
    correlation_removed_features = sorted(
        set(var_features).difference(dynamic_selected)
    )
...
def resolve_boruta_crossfold_features(
    ...
    **kwargs,
    ...
    feature_trace_path = kwargs.get("feature_trace_path")
    corrvar_features = set(kwargs.get("corrvar_features", []) or [])
    ...
    if boruta_crossfold_path.exists():
        ...
        record_removed_features(
            feature_trace_path,
            "boruta",
            sorted(corrvar_features.difference(features)),
        )
    ...
    record_removed_features(
        feature_trace_path,
        "boruta",
        sorted(corrvar_features.difference(boruta_crossfold_features)),
    )
...
# Backward-compatible exports for correlation-analysis call sites.
