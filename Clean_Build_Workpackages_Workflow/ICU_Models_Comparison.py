...
from utilitaries.run_config import load_run_config_from_cli

RUN_CONFIG = load_run_config_from_cli()
REFERENCE_MODE = RUN_CONFIG.reference
...
class _TeeStream:
    ...
    def __init__(self, *streams):
        """Initialize a stream multiplexer.
        
        Args:
            *streams: Writable text streams that receive every message."""
        self.streams = streams

    def write(self, message):
        """Write a message to every configured stream.
    ...
    def isatty(self):
        """Return whether any configured stream is interactive.
        
        Returns:
            ``True`` when at least one stream is attached to a terminal."""
        return any(getattr(stream, "isatty", lambda: False)() for stream in self.streams)

    def fileno(self):
        """Return the file descriptor of the primary stream.
        
        Returns:
            The primary stream file descriptor."""
        return self.streams[0].fileno()

    @property
    def encoding(self):
        """Return the primary stream text encoding.
        
        Returns:
            The detected encoding, or UTF-8 when unavailable."""
...
print(
    "[CONFIG] "
    f"modes={list(RUN_CONFIG.modes)}, "
    f"experiments={RUN_CONFIG.experiment_count()}, "
    f"seed={RUN_CONFIG.seed}, "
    f"figure_format={RUN_CONFIG.figure_output_format}",
    flush=True,
)
...
import utilitaries.figures.performance as sfu
import utilitaries.figures.comparison as comparison_figures
...
from utilitaries.figures.output import (
    resolve_figure_path,
    save_figure as save_figure_file,
)
...
mode_names = RUN_CONFIG.modes
RUN_COMPARISON = RUN_CONFIG.run_comparison
RUN_TRAINING = RUN_CONFIG.run_training
RUN_LASSO = RUN_CONFIG.run_lasso
RUN_INTERPRETABILITY = RUN_CONFIG.run_interpretability
RUN_TEST = RUN_CONFIG.run_test
FIGURE_OUTPUT_FORMAT = RUN_CONFIG.figure_output_format
POPULATION = RUN_CONFIG.population
SAVE_FIGURE = RUN_CONFIG.save_figure
TRANSPARENT = RUN_CONFIG.transparent
DATASET_PATH = RUN_CONFIG.dataset_path
TARGET_COLUMNS = RUN_CONFIG.target_columns
EXPERIMENT_INDEX = 0
TOTAL_EXPERIMENTS = RUN_CONFIG.experiment_count()
print(f"[PIPELINE] {TOTAL_EXPERIMENTS} experiment(s) scheduled.", flush=True)
mode_duplicates = "prio_last"
keep_duplicates = RUN_CONFIG.keep_duplicates
for mode_run in mode_names:
    mode_config = RUN_CONFIG.profiles[mode_run]
    mode_duplicates = mode_config.mode_duplicates
    WINDOWING_MODE = mode_config.windowing_mode
    target_labels = mode_config.target_labels
    model_names = mode_config.model_names
    stratify_modes = mode_config.stratify_modes
    optuna_run_options = (
        (True,)
        if REFERENCE_MODE
        else mode_config.optuna_run_options
    )
    feature_modes = mode_config.feature_modes
    balancing_methods = mode_config.balancing_methods
                            ...
                            try:
                                ...
                                if config_mode.mode == "windows":
                                    ...
                                    valid_patient_ids = (
                                        patient_row_counts
                                        .filter(
                                            pl.col("len") == expected_length
                                        )
                                        .select(patient_col)
                                    )
                                    df_clean_2 = df_clean_1.join(
                                        valid_patient_ids,
                                        on=patient_col,
                                        how="inner",
                                    )
                                    print(
                                        "[WINDOWS FILTER] "
                                        f"required_length={expected_length}, "
                                        f"patients before filter="
                                        f"{patient_row_counts.height}, "
                                        f"patients retained="
                                        f"{valid_patient_ids.height}",
                                        flush=True,
                                    )
                                    if df_clean_2.is_empty():
                                        raise ValueError(
                                            "No patient has exactly "
                                            f"{expected_length} rows."
                                        )
                                else:
                                    observed_lengths = sorted(
                                        patient_row_counts["len"]
                                        .unique()
                                        .to_list()
                                    )
                                    if len(observed_lengths) != 1:
                                        ...
                                        raise ValueError(
                                            "All patients must have the same "
                                            "number of rows in resampling "
                                            "mode. Observed length "
                                            "distribution: "
                                            f"{length_distribution}"
                                        )
                                    expected_length = int(observed_lengths[0])
                                    df_clean_2 = df_clean_1
                                    print(
                                        "[RESAMPLING LENGTH] "
                                        f"all {patient_row_counts.height} "
                                        "patients contain "
                                        f"{expected_length} rows.",
                                        flush=True,
                                    )

                                df_clean_2 = df_clean_2.sort(
                                    patient_col,
                                    time_col,
                                )
                                ...
                                feature_trace_path = None
                                if config_models.extraction_type == "TSFEL":
                                    compare_figs_dir = exp.get_compare_figs_path()
                                    feature_trace_path = (
                                        compare_figs_dir
                                        / "feature_selection_trace.json"
                                    )
                                    ...
                                    if threshold_path.exists():
                                        ...
                                        corr_results = feature_analysis_figures.generate_correlation_analysis(
                                            corr_df,
                                    ...
                                    boruta_cached = (
                                        config_boruta
                                        and boruta_crossfold_path.exists()
                                        and feature_trace_path.exists()
                                    )
                                    ...
                                    if boruta_cached:
                                        ...
                                        train_corrvar, test_corrvar, corrvar_features = (
                                            extract_feat.filtrage_corr_var(
                                                ...
                                                feature_trace_path=feature_trace_path,
                                            )
                                        )
                                        print(
                                    ...
                                    ] = compare_figs_dir / "log_variable_number.json"
                                ...
                                HYPERPARAMS_FILE = output_directory / (
                                    "ref_hyperparameters.json"
                                    if REFERENCE_MODE
                                    else "best_hyperparameters.json"
                                )
                                ...
                                if REFERENCE_MODE and mode_run != "score":
                                    ...
                                    try:
                                        with HYPERPARAMS_FILE.open("r", encoding="utf-8") as config_file:
                                            saved_configs = json.load(config_file)
                                    except (json.JSONDecodeError, OSError) as error:
                                        raise ValueError(
                                            f"[REFERENCE] Unable to read {HYPERPARAMS_FILE}: {error}"
                                        ) from error

                                    if (
                                        not isinstance(saved_configs, dict)
                                        or not isinstance(saved_configs.get(model_name), dict)
                                        or not saved_configs[model_name]
                                    ):
                                        raise ValueError(
                                            f"[REFERENCE] Missing or invalid parameters for "
                                            f"'{model_name}' in {HYPERPARAMS_FILE}"
                                        )

                                    print(
                                        f"[REFERENCE] Loading parameters for {model_name} "
                                        f"from '{HYPERPARAMS_FILE}'. Optuna search disabled."
                                    )
                                ...
                                if RUN_OPTUNA and RUN_TRAINING and not REFERENCE_MODE:
                                ...
                                if RUN_LASSO and config_models.models_name == 'Logistic Regression Lasso TSFEL':
                                    ...
                                    if all(path.exists() for path in existing_lasso_paths):
                                        ...
                                        def process_single_fold(fold_index):
                                            """Compute the Lasso coefficient path for one fold.
                                            
                                            Args:
                                                fold_index: Zero-based cross-validation fold index.
                                            
                                            Returns:
                                                A tuple containing sorted regularization strengths, coefficient paths,
                                                the fitted model strength, and the number of features."""
                                ...
                                if RUN_COMPARISON:
                                    ...
                                    if comparisons:
                                        ...
                                        comparison_figures.old_generate_comparative_report(
                                            comparisons,
                                ...
                                if REFERENCE_MODE:
                                    update_progress(
                                        f"Experiment {EXPERIMENT_INDEX} failed — reference run aborted"
                                    )
                                    raise
