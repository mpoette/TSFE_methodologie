...
def plot_aggregated_odds_ratios(
    ...
    transparent: bool = False,
    top_n: int = 15,
    title: str = "Importance cumulée par variable clinique"
):
    df = pd.read_csv(csv_file)
...
def shap_tsfel_importance_matrix(
    ...
    *,
    ...
    if "detailed" not in shap_results:
        raise KeyError(
            "shap_results doit contenir la clé 'detailed'."
        )
    ...
    if mean_shap_values.ndim != 2:
        raise ValueError(
            "detailed['mean_shap_values'] doit être une matrice 2D."
        )

    if mean_shap_values.shape[1] != len(feature_names):
        raise ValueError(
            "Le nombre de colonnes SHAP ne correspond pas "
            "au nombre de feature_names."
        )
    ...
    if not rows:
        raise ValueError(
            "Aucune caractéristique TSFEL compatible n'a été identifiée."
        )
    ...
    if top_raw_variables is not None:
        ...
        if top_raw_variables <= 0:
            raise ValueError(
                "top_raw_variables doit être > 0 ou None."
            )
    ...
    if top_descriptors is not None:
        ...
        if top_descriptors <= 0:
            raise ValueError(
                "top_descriptors doit être > 0 ou None."
            )
    ...
    # --------------------------------------------------------------
    # Gray overlay only on absent cells
    # --------------------------------------------------------------

    from matplotlib.colors import ListedColormap

    gray_overlay = np.ma.masked_where(
        selected_mask,
